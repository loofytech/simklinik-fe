...
export default function Inspection() {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    {title: 'AKSI', dataIndex: 'id', sorter: false, render: (value, record) => {
      return (<>
        <Link
          href={"/dashboard/public-service/inspection/" + value}
          type="button"
  ...
  const fetchData = async (page?: any) => {
    const request = await fetch(`/api/screening/action?page=${page ?? 1}`);
  ...
  return (<AppLayout>
    <div className="bg-white">
      <div className="p-3 flex items-center justify-between text-xl">
        <span className="font-bold">Pemeriksaan Pasien</span>
