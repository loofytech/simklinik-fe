...
export default function Role() {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    {title: 'NAMA ROLE', dataIndex: 'role_name', className: "w-8/12", sorter: false, render: (value, record) => {
      return `${value}`;
    }},
  ...
  return (<AppLayout>
    <div className="bg-white">
      <div className="p-3">
        <button className="px-4 py-2 bg-primary text-white rounded text-sm" type="button" onClick={() => setModalCreate(true)}>Tambah Role</button>
      </div>
      <Table
        bordered
        columns={columns}
