...
export default function Education() {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    {title: 'NAMA', dataIndex: 'education_name', className: "w-8/12", sorter: false, render: (value, record) => {
      return `${value}`;
    }},
  ...
  const fetchData = async (page?: any) => {
    const request = await fetch(`/api/education?page=${page ?? 1}`);
  ...
  const postData = async (event: any) => {
    ...
    const request = await fetch(`/api/education`, {
      ...
      body: JSON.stringify({
        education_name: name,
        education_slug: name.split(" ").join("_").toLowerCase()
      })
    });
    ...
    >
      <form onSubmit={postData} className="mt-3">
        <div className="mb-3 flex flex-col gap-1.5">
          <label htmlFor="name" className="font-bold">Pendidikan</label>
          <input
            type="text"
