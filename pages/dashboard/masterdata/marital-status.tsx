...
export default function MaritalStatus() {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    {title: 'NAMA', dataIndex: 'marital_name', className: "w-8/12", sorter: false, render: (value, record) => {
      return `${value}`;
    }},
  ...
  const fetchData = async (page?: any) => {
    const request = await fetch(`/api/marital-status?page=${page ?? 1}`);
  ...
  const postData = async (event: any) => {
    ...
    const request = await fetch(`/api/marital-status`, {
      ...
      body: JSON.stringify({
        marital_name: name,
        marital_slug: name.split(" ").join("_").toLowerCase()
      })
    });
  ...
  return (<AppLayout>
    <div className="bg-white">
      <div className="p-3">
        <button className="px-4 py-2 bg-primary text-white rounded text-sm" type="button" onClick={() => setModalCreate(true)}>Tambah Status Pernikahan</button>
      </div>
      <Table
        bordered
        columns={columns}
    ...
    >
      <form onSubmit={postData} className="mt-3">
        <div className="mb-3 flex flex-col gap-1.5">
          <label htmlFor="name" className="font-bold">Status Pernikahan</label>
          <input
            type="text"
